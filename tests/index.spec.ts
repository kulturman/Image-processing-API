...
describe('GET /api/images', () => {
  it('Should create a thumbnail if image exists and is processable', async () => {
    ...
    const thumbnailPath = imageProcessingService.generateThumbnailPath(
      'test.png',
  ...
  it('Should return a 400 if image not found', async () => {
    await request(app)
      .get('/api/images?filename=inexistent.png&width=300&height=300')
      .expect(400);
  });

  it('Should return a 400 one of the required 3 query params is missing', async () => {
    await request(app)
      .get('/api/images?filename=test.png&width=300')
      .expect(400);
  });
});
